// frontend/src/components/ScheduleTableView.js
...
export default function ScheduleTableView({ departmentId, view }) {
  ...
  // 2️⃣ Загрузка справочников + отпусков
  useEffect(() => {
    Promise.all([
      ...
      const vmap = {};
      vacR.data.forEach(v => {
        vmap[v.employee] = vmap[v.employee] || [];
  ...
  const onDrop = (e, empId, date) => {
    ...
    // Убираем старую из UI
    setSchedule(prev => {
      const next = { ...prev }; delete next[cell.empId].byDate[cell.date]; return next;
    });
    // Переносим в бэке
    axios.delete(`/api/shifts/${cell.id}/`)
      .then(() => axios.post('/api/schedules/bulk/', {
        entries:[{
          employee: cell.empId,
          date,
          shift_type: shiftTypes.find(t=>t.color===cell.color).id
        }]
      }))
      .then(({ data:[newShift] }) => {
  ...
  const onSelectChange = (empId,date,stId) => {
    if (!stId) {
      ...
      if (cell) axios.delete(`/api/shifts/${cell.id}/`)
        .then(() => setSchedule(prev=>{
          const nx={...prev}; delete nx[empId].byDate[date]; return nx;
        }));
    } else {
  ...
  // Helpers
  const inVacation = (eId,d) => (vacations[eId]||[])
    .some(v=>d>=v.start && d<=v.end);
  const isBirthdayCell = (birth,d) =>
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex items-center mb-4 space-x-2">
        <button
          onClick={()=>setCurrent(c=>c.subtract(step[1],step[0]))}
        ...
        <button
          onClick={()=>setCurrent(c=>c.add(step[1],step[0]))}
          className="px-2 py-1 bg-gray-200 rounded hover:bg-gray-300"
        >›</button>
      </div>
        ...
        <table className="min-w-full table-fixed border-collapse">
          <thead className="bg-gray-100">
            <tr>
              ...
              {dates.map(d=>{
                const dt = dayjs(d);
                const weekend = dt.day()===0||dt.day()===6;
                const holiday = !!hd.isHoliday(dt.toDate());
                const label = view==='month'
                  ? dt.format('D')
                  : view==='week'
                    ? `${dt.format('D')} ${WEEKDAYS[dt.day()]}`
                    : dt.format('DD.MM.YYYY');
                return (
                  <th key={d}
                      className={`p-2 border text-center text-sm ${weekend||holiday?'bg-gray-200':''}`}>
                    {label}
                  </th>
                );
              })}
              ...
              return (
                <tr key={emp.id} className="hover:bg-gray-50">
                  ...
                  {dates.map(date=>{
                    ...
                    const vac  = inVacation(emp.id,date);
                    ...
                    return (
                      ...
                      >
                        {/* 🎂 в ячейке ДР */}
                        {isBirthdayCell(emp.birth_date, date) && (
